export default {
      elements: [
      {
        type: 'svg',
        name: 'Monkey 404',
        className: 'me404',
        css: 'width: 1000px; height: 480px; position: absolute; top: 50%; left: 50%; margin-left: -400px; margin-top: -240px;',
        viewBox: '0 0 1000 480',
        elements: [
          {
            name: 'cloud',
            type: 'path',
            linkedAnimationKeyPath: [0],
            id: 'cloud',
            className: 'st0',
            css: 'fill-rule: evenodd; clip-rule: evenodd; fill: #E8EBED;',
            d: 'M658.4,345.2c-10.9,0-19.7-8.8-19.7-19.7c0-10.9,8.8-19.7,19.7-19.7h50.1c9.9-1.5,17.5-10,17.5-20.3 c0-11.4-9.2-20.6-20.6-20.6v-0.2H633c-11.4,0-20.6-6.7-20.6-18.1c0-11.4,9.2-19.3,20.6-19.3h70.4l2-0.2c7.3-3.1,12.5-11,12.5-19.5 c0-8.5-4.2-16.7-11.4-19.2l-2.5-0.3h-11.3c-11.9,0-21.6-8.9-21.6-19.9c0-11,9.7-19.9,21.6-19.9h15.8l1.4-0.3 c8.6-2.5,14.8-10.1,14.8-19.5c0-11.4-9.2-20.6-20.6-20.6h-1.2h-69.2H382.5c-19.8-0.9-19.9-15.9-19.8-17.8c0-0.1,0-0.1,0-0.2 c0-9.9-8.1-18-18-18h-93.5c-9.9,0-18,8.1-18,18c0,9.4,7.2,17.1,16.3,17.9h9.3c0.2,0,0,0,0.6,0l0.5,0l0.4,0l0.2,0 c10.1,0.9,18,9.3,18,19.6c0,10.9-8.8,19.7-19.7,19.7h-70.7c-11.3,0-20.5,9.2-20.5,20.6c0,11.3,9.1,20.5,20.4,20.6h48.8 c10.3,0,18.7,8.4,18.7,18.7c0,10.3-8.4,18.7-18.7,18.7h-23.2c-11.3,0.1-20.4,9.2-20.4,20.6c0,11.3,9.2,20.5,20.5,20.6h6.3 c10.7,0,19.3,8.7,19.3,19.3c0,10.7-7.8,19.3-18.4,19.3l-1.5,0l-2.8,0.4c-7.3,3.1-11.8,11-11.5,18.9c0.3,8.5,4.2,16.5,11.7,19.6 c1.1,0.7,3.4,0.9,4.4,0.9h4.5H296h19.7c3.9,0.5,8.2,4.2,7.4,10.4c0,0.4,0,0.8,0.1,1.1c0,0.5-0.1,1-0.1,1.5c0,9.7,7.9,17.5,17.5,17.5 h60.2c9.7,0,17.5-7.9,17.5-17.5c0-0.4,0-0.8-0.1-1.2c0.1-0.3,0-0.7,0.1-1.1c0.3-6.5,6.4-10.9,10.6-10.8h110.1 c8.5,0,16.9,6.6,16.9,14.8c0,8.2,6.6,14.8,14.8,14.8h92.6c8.2,0,14.8-6.6,14.8-14.8c0-8.2-6.6-14.8-14.8-14.8 M332.8,187.1h-21.2 c-11.4,0-20.6-9.2-20.6-20.6c0-11.4,9.2-20.6,20.6-20.6h21.2c11.4,0,20.6,9.2,20.6,20.6C353.3,177.9,344.1,187.1,332.8,187.1z'
          },
          {
            name: 'moon',
            linkedAnimationKeyPath: [1],
            type: 'g',
            id: 'moon',
            css: 'transform-origin: 640px 81px;',
            elements: [
              {
                name: 'moon_body',
                type: 'path',
                className: 'st5',
                d: 'M641,34c26,0,47,21,47,47s-21,47-47,47s-47-21-47-47S615,34,641,34z',
                css: 'fill-rule: evenodd; clip-rule: evenodd; fill: #FFFFFF; stroke: #89949B; stroke-width: 3; stroke-linecap: round; stroke-linejoin: round; stroke-miterlimit: 10;',
              },
              {
                name: 'moon_shades',
                type: 'path',
                className: 'st0',
                d: 'M622.5,55.9c1.3,2.3,0,5.8-3.1,7.7c-3,2-6.6,1.7-7.9-0.6c-1.3-2.3,0-5.8,3.1-7.7 C617.6,53.3,621.1,53.6,622.5,55.9z M628.8,94.1c-4.1-6.1-11.6-9-16.7-6.4c-5.1,2.6-5.9,9.6-1.7,15.7c4.1,6.1,11.6,9,16.7,6.4 C632.2,107.2,632.9,100.2,628.8,94.1z M644.5,109c-3.6,0-6.5,2.2-6.5,5s2.9,5,6.5,5s6.5-2.2,6.5-5S648.1,109,644.5,109z M645.7,95.8c-2.3-1.2-5-0.5-6,1.4c-1,2,0,4.5,2.3,5.7c2.3,1.2,5,0.5,6-1.4C649,99.6,648,97,645.7,95.8z M686.5,81 c0-25.1-20.4-45.5-45.5-45.5c-16.1,0-30.2,8.4-38.3,21c7.9-5.9,17.7-9.5,28.3-9.5c26,0,47,21,47,47c0,6.3-1.3,12.3-3.5,17.8 C681.9,103.6,686.5,92.8,686.5,81z',
                css: 'fill-rule: evenodd; clip-rule: evenodd; fill: #E8EBED;'
              }
            ]
          }
        ]
      }
      ],
      animations: [
        {
          name: 'cloud-anim',
          animationProperties : {
            animationDirection : 'normal',
            duration : '9s',
            iteration : 'infinite',
            timingFunction : 'ease-in-out'
          },
          keyframes: [
            {
              position: 0,
              css: 'opacity: 0.3; left: 160px; transform:skewX(20deg);'
            },
            {
              position: 65,
              css: 'opacity: 1; left: 114px; transform:skewX(0deg);'
            },
            {
              position: 100,
              css: 'opacity: 0.3; left: 160px; transform:skewX(20deg);'
            }
          ]
        },
        {
          name: 'moon-float-anim',
          animationProperties: {
            animationDirection: 'normal',
            duration: '6s',
            iteration: 'infinite',
            timingFunction: 'ease-in-out'
          },
          keyframes: [
            {
              position: 0,
              css: 'transform: translate(0, 20px) scale(1.1);'
            },
            {
              position: 65,
              css: 'transform: translate(0, 0px) scale(1.0);'
            },
            {
              position: 100,
              css: 'transform: translate(0, 20px) scale(1.1);'
            }
          ]
        }
      ],
      previewContentCSS : '',
      rawCSS : ".me404 { width: 1000px; height: 480px; position: absolute; top: 50%; left: 50%; margin-left: -400px; margin-top: -240px; } .st0 { fill-rule: evenodd; clip-rule: evenodd; fill: #E8EBED; } #moon { transform-origin: 640px 81px; } .st5{fill-rule:evenodd;clip-rule:evenodd;fill:#FFFFFF;stroke:#89949B;stroke-width:3;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:10;} .st0{fill-rule:evenodd;clip-rule:evenodd;fill:#E8EBED;}"
};









